...
export function FooterSection() {
  return (
    <footer className="py-20 px-6 border-t border-gray-200 bg-[#FCFBF8] relative overflow-hidden">
      <div className="max-w-[1300px] mx-auto relative z-10">
        <div className="grid lg:grid-cols-6 gap-16 mb-20">
          ...
          <div className="lg:col-span-2">
            <Link href="/" className="flex items-center gap-3 mb-6">
              ...
              <span className="font-comic text-2xl font-bold tracking-tight text-gray-800">FormForge</span>
            </Link>
            <p className="text-lg text-gray-600 font-comic mb-8 max-w-sm leading-relaxed">
          ...
          <div className="lg:col-span-4 grid grid-cols-2 md:grid-cols-4 gap-10">
            {Object.entries(links).map(([title, items]) => (
              <div key={title}>
                <h4 className="font-comic text-lg font-bold text-gray-800 mb-6 tracking-tight">{title}</h4>
                <ul className="space-y-3">
                  {items.map((item) => (
                    <li key={item}>
                      <a href="#" className="text-base font-comic text-gray-600 hover:text-gray-900 transition-colors">{item}</a>
                    </li>
                  ))}
                </ul>
              </div>
        ...
        <div className="border-t border-gray-200 pt-8 flex flex-col md:flex-row items-center justify-between gap-6">
          <p className="text-sm font-comic text-gray-500 tracking-wide">© 2026 FormForge. Crafted with care.</p>
          <div className="flex items-center gap-2 px-3 py-1.5 border border-gray-200 bg-white shadow-sm rounded-lg">
            ...
            <span className="font-comic text-sm text-gray-600">All systems operational</span>
          </div>
        </div>
      </div>
    </footer>
